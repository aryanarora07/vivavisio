...
const CreatePost = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (form.prompt && form.photo) {
      setLoading(true);
      try {
        const response = await fetch('https://vivavisiobackend.onrender.com/api/v1/post', {
          method: 'POST',
          headers: {
  ...
  const generateImage = async () => {
    if (form.prompt) {
      try {
        ...
        const response = await fetch("https://vivavisiobackend.onrender.com/api/v1/dalle", {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
    ...
    <section className="max-w-7xl mx-auto ">
      <div>
        ...
        <p className="mt-2 text-[#666e75] text-[14px] max-w-[700px]">
          Create imaginative and visually stunning images through VivaVisio AI
